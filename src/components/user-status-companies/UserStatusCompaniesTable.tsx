...
interface Props {
  rows: UserStatusCompany[];
  sort: { column: UserStatusCompanySortColumn; order: SortOrder } | null;
  onSort: (column: UserStatusCompanySortColumn) => void;
  onEdit: (company: UserStatusCompany) => void;
  onStatusChange: (company: UserStatusCompany) => void;
}

export function UserStatusCompaniesTable({ rows, sort, onSort, onEdit, onStatusChange }: Props) {
  if (rows.length === 0) {
    return <EmptyState title="Sin empresas" description="No hay empresas que coincidan con los filtros aplicados." />;
  }
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[760px] border-collapse text-left text-sm">
          <thead className="bg-muted text-xs uppercase text-muted-foreground">
            <tr>
              <SortableHeader column="name" label="Empresa" sort={sort} onSort={onSort} />
              <SortableHeader column="active" label="Estado" sort={sort} onSort={onSort} />
              <SortableHeader column="userCount" label="Usuarios asociados" sort={sort} onSort={onSort} />
            ...
            {rows.map((company) => (
              <tr key={company.id} className="hover:bg-muted/40">
                ...
                <td className="px-4 py-3">
                  <Badge variant={company.active ? "success" : "default"}>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      aria-label={`${company.active ? "Desactivar" : "Activar"} ${company.name}`}
                    >
...
function SortableHeader({
  ...
  column: UserStatusCompanySortColumn;
  label: string;
  sort: Props["sort"];
  onSort: Props["onSort"];
}) {
